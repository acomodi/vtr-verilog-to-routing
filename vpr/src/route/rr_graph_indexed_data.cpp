...
static void load_rr_indexed_data_T_values() {
    ...
    for (size_t cost_index = CHANX_COST_INDEX_START;
        ...
        if (num_nodes_of_index[cost_index] == 0) { /* Segments don't exist. */
            ...
            VTR_LOG("\nCost Index: %d\tSegment info: %s\n", cost_index, segment_inf[rr_indexed_data[cost_index].seg_index].name.c_str());

            VTR_LOG("\tNode R Total\n");
            print_histogram(R_total_histogram);
            VTR_LOG("\n\tNode C Total\n");
            print_histogram(C_total_histogram);
            VTR_LOG("\n\tSwitch R Total\n");
            print_histogram(switch_R_total_histogram);
            VTR_LOG("\n\tSwitch T Total\n");
            print_histogram(switch_T_total_histogram);
            VTR_LOG("\n\tSwitch Cinternal Total\n");
            print_histogram(switch_Cinternal_total_histogram);
